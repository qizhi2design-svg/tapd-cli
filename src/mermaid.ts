...
export async function convertMermaidBlocks(
  markdown: string,
  options: ConvertMermaidBlocksOptions
...
export function renderMermaidTemplate(
  templateContent: string,
  images: MermaidImage[],
  imageSrc: (image: MermaidImage) => string | undefined
): string {
...
export async function renderMermaidPngBase64(diagram: string, renderBaseUrl = "https://mermaid.ink"): Promise<string> {
  ...
  if (bytes.length === 0) throw new Error("Mermaid 图片渲染失败：返回内容为空");
