...
export function registerLogin(program: import("commander").Command): void {
  program
    .command("login")
    ...
    .action(async (options: {
      ...
      if (mode === "personal") {
        ...
        if (workspace.companyId) success(`已绑定 company_id：${workspace.companyId}`);
...
export function registerLogout(program: import("commander").Command): void {
  program
    .command("logout")
