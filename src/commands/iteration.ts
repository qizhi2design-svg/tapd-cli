...
function formatStoryStatusSummary(stories: Array<{ status?: string }>, statusMap: Record<string, string>): string {
...
export function registerIteration(program: import("commander").Command): void {
...
`);
  ...
  iteration
    ...
    .action(async (options: IterationListOptions) => {
      ...
      const iterations = await withSpinner(
        spinner,
        async () => {
  ...
  iteration
    ...
    .action(async (iterationId: string, options: { workspaceId?: string }) => {
      ...
      const [iterationData, stories, storyStatusMap] = await withSpinner(
        spinner,
        async () => Promise.all([
...
`)
    .action(async (iterationId: string, options: IterationTasksOptions) => {
      const workspace = await resolveWorkspaceContext(process.cwd(), options.workspaceId);
      workspaceBanner(workspace);
      const workspaceId = workspace.id;
      const client = new TapdClient();
      ...
      const [iterationData, taskResult] = await withSpinner(
        spinner,
        async () => Promise.all([
          client.getIteration(token, workspaceId, iterationId),
